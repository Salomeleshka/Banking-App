...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const formattedMov = fromatCur(mov, acc.locale, acc.currency);
...
const calcDisplayBalance = function (acc) {
  ...
  labelBalance.textContent = fromatCur(acc.balance, acc.locale, acc.currency);
};
...
const calcDisplaySummary = function (acc) {
  ...
  labelSumIn.textContent = fromatCur(incomes, acc.locale, acc.currency);

  const out = acc.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = fromatCur(Math.abs(out), acc.locale, acc.currency);
